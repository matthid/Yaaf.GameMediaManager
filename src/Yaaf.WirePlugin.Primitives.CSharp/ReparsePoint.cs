// ----------------------------------------------------------------------------
// This file (ReparsePoint.cs) is subject to the terms and conditions defined in
...
namespace ReparsePoints
{
    ...
    public class ReparsePoint
    {
        ...
        private const uint IO_REPARSE_TAG_SYMLINK = 0xA000000C;
                           // SYMLINK or SYMLINKD (see http://wesnerm.blogs.com/net_undocumented/2006/10/index.html)
        ...
        public ReparsePoint(string path)
        {
            ...
            if (success)
            {
                success = LookupPrivilegeValue(null, SE_BACKUP_NAME, out tokenPrivileges.Privileges[0].Luid);
                    ...
                    if (success)
                    {
                        ...
                        if (buffer.ReparseTag == IO_REPARSE_TAG_SYMLINK)
                        {
                            ...
                            subsString = new string(
                                buffer.ReparseTarget, (buffer.SubsNameOffset / 2 + 2), buffer.SubsNameLength / 2);
                            printString = new string(
                                buffer.ReparseTarget, (buffer.PrintNameOffset / 2 + 2), buffer.PrintNameLength / 2);
                        ...
                        else if (buffer.ReparseTag == IO_REPARSE_TAG_MOUNT_POINT)
                        {
                            ...
                            subsString = new string(
                                buffer.ReparseTarget, buffer.SubsNameOffset / 2, buffer.SubsNameLength / 2);
                            printString = new string(
                                buffer.ReparseTarget, buffer.PrintNameOffset / 2, buffer.PrintNameLength / 2);
                            tag = subsString.StartsWith(@"\??\Volume") ? TagType.MountPoint : TagType.JunctionPoint;
                        }
                        Debug.Assert(
                            !(string.IsNullOrEmpty(subsString) && string.IsNullOrEmpty(printString)),
                            "Failed to retrieve parse point");
                        ...
                        else
                        {
                            ...
                            Debug.Assert(
                                (normalisedTarget.StartsWith(@"\??\")
                                 && (normalisedTarget[5] == ':' || normalisedTarget.StartsWith(@"\??\Volume"))
                                 || (!normalisedTarget.StartsWith(@"\??\") && normalisedTarget[1] != ':')),
                                "Malformed subsString");
                            // Junction points must be absolute
                            Debug.Assert(
                                buffer.ReparseTag == IO_REPARSE_TAG_SYMLINK
                                || normalisedTarget.StartsWith(@"\??\Volume") || normalisedTarget[1] == ':',
        ...
        private struct LUID
        {
            public readonly UInt32 LowPart;

            public readonly Int32 HighPart;
        }
        ...
        #region Nested type: REPARSE_DATA_BUFFER
        ...
        private struct REPARSE_DATA_BUFFER
        {
            ...
            public readonly short ReparseDataLength;

            public readonly short Reserved;

            public readonly short SubsNameOffset;

            public readonly short SubsNameLength;

            public readonly short PrintNameOffset;

            public readonly short PrintNameLength;

            [MarshalAs(UnmanagedType.ByValArray, SizeConst = MAXIMUM_REPARSE_DATA_BUFFER_SIZE)]
            public readonly char[] ReparseTarget;
        }
